              ...
              {(() => {
                ...
                return (
                  <button onClick={() => navigate({ screen: "tickets" })}
                    title={`${openTickets.length} open ticket${openTickets.length === 1 ? "" : "s"} — click to view`}
                    style={{
                      display: "inline-flex", alignItems: "center", gap: 6,
                      padding: "3px 10px", borderRadius: 999,
                      background: "var(--warning-bg)",
                      border: "1px solid color-mix(in oklab, var(--color-warning-500) 30%, transparent)",
                      color: "var(--color-warning-700)", fontSize: 11.5, fontWeight: 500,
    ...
    <window.Card title="System settings"
      ...
      <div style={{ display: "grid",
        ...
        <div>
          ...
          <SettingRow label="Input method" mono value={t.settings.inputMethod}
            pending={pending["set.ime"]}
            onEdit={() => openEdit({
              id: "set.ime", label: "Input method",
              kind: "select", options: SETTING_OPTIONS.inputMethod,
              from: t.settings.inputMethod, to: t.settings.inputMethod,
            })} />
