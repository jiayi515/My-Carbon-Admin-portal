...
function NewTicketScreen({ navigate, presetSn }) {
  ...
  // Take the first sentence (or first ~80 chars on a word boundary) of the
  // description so the ticket list still has a readable headline.
  const summariseTitle = (desc) => {
    const flat = desc.replace(/\s+/g, " ").trim();
    if (!flat) return "(Untitled ticket)";
    const sentenceEnd = flat.search(/[。.!?！？]\s|[。.!?！？]$/);
    let cut = sentenceEnd > 0 ? flat.slice(0, sentenceEnd) : flat;
    if (cut.length > 80) {
      const slice = cut.slice(0, 80);
      const lastSpace = slice.lastIndexOf(" ");
      cut = (lastSpace > 40 ? slice.slice(0, lastSpace) : slice) + "…";
    }
    return cut;
  };
  ...
  const submit = () => {
    ...
    const finalTitle = title.trim() || summariseTitle(trimmedDesc);
    const newTicket = {
      id: `T-2026-${nextNum}`,
        ...
        <div className="page-content--narrow" style={{ padding: "var(--space-6)" }}>
          <window.Card padding={20}>
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              hint={title.trim() ?
              <span><span className="mono">{title.length}</span> / 120 — short summary of the issue.</span> :
              <span>Optional — leave blank to auto-summarise from the description below.</span>}>
                <window.Input value={title} onChange={(e) => setTitle(e.target.value.slice(0, 120))}
                placeholder="Leave blank to auto-summarise from description" />
              </window.Field>
