...
// ─ App-level publish state (three-state machine) ────────
// ISVs publish directly — no Admin review. Apps live in the publisher's
// LOCAL pool until they're pushed to the PUBLIC pool, where ISO companies
// can subscribe and roll the snapshot out to their merchants.
//
//   private    — Created. Visible only inside the publishing org.
//                · Pure-ISV tenants land here briefly after creation,
//                  then auto-promote to "published" once the first
//                  version finishes uploading.
//                · ISV+ISO tenants stay here unless an operator
//                  explicitly publishes to the public pool. Used for
//                  apps the org wants to deploy to its own merchants
//                  only, without listing in the marketplace.
//   published  — Live in the public pool. ISO companies can subscribe.
//   unpublished — Withdrawn from the public pool. Existing subscribers
//                 keep their snapshot and the app keeps running on
//                 already-rolled-out terminals — only NEW subscriptions /
//                 device pushes are blocked. Re-publish is one click;
//                 no review.
const APP_STATUS = {
  "private":     { label: "Private",     tone: "neutral", description: "In your local pool — visible only to your organization. Push to the public pool when ready to list for ISO subscribers." },
  "published":   { label: "Published",   tone: "success", description: "Live in the public pool. ISO companies can subscribe and push to their merchant fleets." },
  "unpublished": { label: "Unpublished", tone: "warning", description: "Withdrawn from the public pool. Existing subscribers keep using their current snapshot but new devices can't pick it up. Re-publish anytime — no review needed." },
};
...
const VERSION_STATUS = {
  ...
  "draft":       { label: "Draft",       tone: "neutral" }, // wizard save-as-draft only; not commonly shown
};

// Back-compat aliases for older code paths that still reference the
// pre-three-state status names. They all collapse onto "private" so any
// stale data still renders something sensible.
APP_STATUS["not-published"]   = APP_STATUS.private;
APP_STATUS["awaiting-review"] = APP_STATUS.private;
APP_STATUS["rejected"]        = APP_STATUS.private;
...
const APPS = [
  ...
  // until at least one version exists).
  {
    id: "tablemanager",
    name: "Table Manager",
    package: "com.acme.tablemanager",
    iconId: "tablemanager",
    category: "Food & Beverage",
    description: "Floor plans, table assignments and wait-list tracking for full-service restaurants. Pairs with Acme POS for course-firing.",
    devices: ["N950", "X800", "S90"],
    status: "private",
    orientations: ["portrait", "landscape"],
    subscriberIds: [],
...
// owned by Summit Retail Co. (which holds both ISV and ISO contracts); the
// `delivery` app is also owned by Summit and stays in the Private state to
// demonstrate the ISV+ISO "internal only" path. The rest are owned by Acme
// Software, our default pure-ISV tenant — their apps auto-publish on first
// version upload.
...
// No Admin review in the new model — the lifecycle is entirely owned by the
// ISV. Each entry: { kind, at, actor, note? }. Kinds:
//   created       — app was registered (metadata-only, no APK yet)
//   version-uploaded — a new version finished uploading
//   published     — app went live in the public pool
//                   (for pure-ISV: auto-fires after first version upload;
//                    for ISV+ISO: fires when operator explicitly publishes)
//   unpublished   — app withdrawn from the public pool
...
const APP_REVIEW_ACTIVITY = {
  ...
  "delivery": [
    // ISV+ISO tenant chose NOT to publish to the public pool on first
    // upload — they keep this app private for internal merchant rollout.
    ...
    { kind: "version-uploaded", at: "May 12, 2026", actor: "L. Tremblay",
      note: "0.9.0-beta uploaded. Operator chose to keep app private — not listed in the public pool." },
  ],
  "timeclock": [
    { kind: "created",   at: "Aug 05, 2023", actor: "L. Tremblay" },
    { kind: "published", at: "Aug 10, 2023", actor: "L. Tremblay",
      note: "Published to public pool." },
  ],
  "giftcard": [
    // ISV+ISO tenant deliberately keeping this private — internal use only.
    { kind: "created",   at: "Apr 28, 2026", actor: "L. Tremblay",
      note: "Internal use only — not listed in public pool." },
    { kind: "version-uploaded", at: "May 04, 2026", actor: "L. Tremblay",
      note: "1.6.0 uploaded for internal Summit rollout." },
  ],
  "tablemanager": [
    { kind: "created", at: "May 13, 2026", actor: "M. Hassan",
      note: "App registered. Upload an APK to publish." },
  ],
};
...
const SUBSCRIBED_APPS = {
  ...
  "northbay": [
    { appId: "pos",       subscribedVersionId: "v29", subscribedAt: "Mar 12, 2026", deployedTerminals:  86 },
    { appId: "inventory", subscribedVersionId: "v11", subscribedAt: "Apr 09, 2026", deployedTerminals:  42 },
    { appId: "loyalty",   subscribedVersionId: "v06", subscribedAt: "Apr 22, 2026", deployedTerminals:  31 },
    { appId: "timeclock", subscribedVersionId: "v14", subscribedAt: "May 02, 2026", deployedTerminals: 124 },
    { appId: "catalog",   subscribedVersionId: "v08", subscribedAt: "Apr 14, 2026", deployedTerminals:  18 },
  ],
  "summit": [
...
const MERCHANT_FLEETS = {
  "northbay": [
    { id: "m-nb-001", name: "Riverside Coffee Co.",     region: "Quebec, CA",  terminals: 14 },
    { id: "m-nb-002", name: "Maplewood Grocery",        region: "Quebec, CA",  terminals:  8 },
    { id: "m-nb-003", name: "Pinecone Pharmacy",        region: "Ontario, CA", terminals: 22 },
    { id: "m-nb-004", name: "Aspen Diner Group",        region: "Quebec, CA",  terminals: 11 },
    { id: "m-nb-005", name: "Lakeshore Hardware",       region: "Ontario, CA", terminals:  6 },
    { id: "m-nb-006", name: "Harbor Liquor Mart",       region: "Nova Scotia", terminals:  9 },
    { id: "m-nb-007", name: "Trillium Books",           region: "Ontario, CA", terminals:  4 },
    { id: "m-nb-008", name: "Birchwood Cafe Chain",     region: "Quebec, CA",  terminals: 17 },
    { id: "m-nb-009", name: "Northern Lights Outdoors", region: "Manitoba",    terminals: 12 },
    { id: "m-nb-010", name: "Cobblestone Bakery",       region: "Quebec, CA",  terminals:  5 },
    { id: "m-nb-011", name: "Frost Bay Convenience",    region: "Nova Scotia", terminals: 18 },
    { id: "m-nb-012", name: "Granite Cliff Apparel",    region: "Ontario, CA", terminals:  7 },
  ],
  "summit": [
    { id: "m-sm-001", name: "Summit Coffee House",      region: "BC, CA",      terminals: 28 },
    { id: "m-sm-002", name: "Cascade Bistro",           region: "BC, CA",      terminals: 19 },
    { id: "m-sm-003", name: "Cedar Park Pharmacy",      region: "Alberta, CA", terminals: 36 },
    { id: "m-sm-004", name: "Glacier Grocers",          region: "BC, CA",      terminals: 47 },
    { id: "m-sm-005", name: "Mountain View Retail",     region: "BC, CA",      terminals: 12 },
    { id: "m-sm-006", name: "Sapphire Lake Resorts",    region: "Alberta, CA", terminals: 24 },
    { id: "m-sm-007", name: "Whitepine Supermarket",    region: "Yukon",       terminals: 15 },
    { id: "m-sm-008", name: "Birch & Stone Cafe",       region: "BC, CA",      terminals:  9 },
    { id: "m-sm-009", name: "Driftwood Diner Co.",      region: "BC, CA",      terminals: 21 },
    { id: "m-sm-010", name: "Iron Range Outfitters",    region: "Alberta, CA", terminals: 14 },
  ],
  "acme-sw": [],
};

// Deterministically pick which merchants in a tenant's fleet have a given app
// configured. The same (tenant, app) pair always returns the same merchants.
function merchantsConfiguredForApp(tenantId, appId) {
  ...
  return fleet.filter((_, i) => ((h + i * 7) % 10) < 7);
}
...
// across re-renders. Mixes "installed", "pending" and "failed" so the UI can
// demonstrate all three states.
function mockTerminalsForMerchant(merchant, version) {
  const count = merchant.terminals;
  // seed hash from merchant id
  let h = 0;
  for (const c of (merchant.id + ":" + version.id)) h = (h * 31 + c.charCodeAt(0)) & 0x7fffffff;
  const terminals = [];
  for (let i = 0; i < count; i++) {
    const r = (h + i * 17) % 100;
    const status = r < 80 ? "installed"
                : r < 92 ? "pending"
                          : "failed";
    const sn = `T${(h + i * 7).toString(36).toUpperCase().slice(0, 6)}${String(i).padStart(3, "0")}`;
    terminals.push({ sn, status });
  }
  return terminals;
}
...
Object.assign(window, {
  DEVICE_MODELS, APP_STATUS, VERSION_STATUS, REVIEW_STATE, SEVERITY, CATEGORIES, CUSTOMERS, ISO_COMPANIES, APPS, SUBSCRIBED_APPS, TENANT_NAMES,
  MERCHANT_FLEETS, merchantsConfiguredForApp, ROLLOUT_DEFAULT_CURVE,
  ROLLOUT_HISTORY, merchantsAlreadyOnVersion, recordRollout,
